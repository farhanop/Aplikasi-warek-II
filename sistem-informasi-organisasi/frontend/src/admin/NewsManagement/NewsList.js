// frontend/src/admin/NewsManagement/NewsList.js
...
function NewsList() {
    ...
    const fetchNews = async () => {
        ...
        setError(null);
        try {
            // Ambil semua berita tanpa filter bahasa untuk manajemen
    ...
    if (loading) {
        return <div className="text-center p-4">Memuat berita...</div>;
    }
    ...
    return (
        <div className="p-4">
            <h2 className="text-3xl font-bold mb-6 text-center">{getTranslation(currentLang, 'manageNews')}</h2>
            <div className="flex justify-end mb-4">
                <Link to="new" className="px-6 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600">
                    {getTranslation(currentLang, 'addNews')}
                </Link>
            </div>

            {news.length === 0 ? (
                <p className="text-center">{getTranslation(currentLang, 'noNewsAvailable')}</p>
            ) : (
